...
def generate_prompt(passengerData):
    ...
    for k in passengerData:
        if k != "PassengerId" and k != "Survived" and passengerData[k] is not None:
...
def formatting_func(example):
    line = generate_prompt(example) + " " + str(example["Survived"]) + "<eos>"
    return line
...
trainer = SFTTrainer(
    ...
    args=TrainingArguments(  # Arguments for training setup.
        ...
        warmup_steps=64,  # Number of steps to gradually increase the learning rate at the beginning of training.
        max_steps=1600,  # Total number of training steps to perform.
        ...
        output_dir="outputs_gemma2_base_eos_ignore_empty_2",  # Directory where training outputs will be saved.
