...
output_dir="outputs_gemma2_base_eos_ignore_empty_2/checkpoint-1600"
...
for row in dataset:
    ...
    response_text = generate_response(model, tokenizer, prompt, device, 32)
    print(response_text)
    final_response = "no prediction"
    for partText in response_text.splitlines():
        if "Survived" in partText:
            final_response = partText
...
with open("submission_gemma_base_eos_ignore_empty_1600.csv", 'w') as csvFile:
