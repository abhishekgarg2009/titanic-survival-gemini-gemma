...
with open("test.csv") as csvFile:
    ...
    for rows in csvRead:
        ...
        for attempt in range(10):
            try:
                response = client.models.generate_content(model="gemini-2.0-flash-thinking-exp-01-21", contents=prompt)
            except:
                time.sleep(10)
            else:
                break
...
with open("submission_gemini_thinking.csv", 'w') as csvFile:
